...
class Gra:
	def __init__(self):
		...
		self.tk = Tk()
		self.tk.title("Pan Patyczek pędzi do wyjścia")
		self.tk.resizable(0,0)
		self.tk.wm_attributes("-topmost",1)
		self.plotno = Canvas(self.tk, width=self.wysokosc_plotna,	\
			height=self.szerokosc_plotna, highlightthickness=0)
		self.plotno.pack()
		self.tk.update()
		...
		for x in range(0,5):
			for y in range(0,5):
				self.plotno.create_image(x*szer, y*wys,	\
					image = self.tlo, anchor="nw")
...
def na_osi_x(wsp1, wsp2):
	if (wsp1.x1 > wsp2.x1 and wsp1.x1 < wsp2.x2)	\
			...
			or (wsp2.x2 > wsp1.x1 and wsp2.x2 < wsp1.x1):
		return True
	else:
		return False
...
def na_osi_y(wsp1, wsp2):
	if (wsp1.y1 > wsp2.y1 and wsp1.y1 < wsp2.y2)	\
			...
			or (wsp2.y2 > wsp1.y1 and wsp2.y2 < wsp1.y1):
		return True
	else:
		return False
...
class DuszekPlatforma(Duszek):
	def __init__(self, gra, obrazek, x, y, szerokosc, wysokosc):
		...
		self.image = gra.plotno.create_image(x, y,	\
			image = self.obrazek, anchor='nw')
		self.wspolrzedne = Coords(x, y,	x + szerokosc, y + wysokosc)
		self.koniec_gry = False

class DuszekPatyczek(Duszek):
	def __init__(self, gra):
		Duszek.__init__(self, gra)
		self.obrazki_lewa = [
			PhotoImage(file="patyczek_L1.gif"),
			PhotoImage(file="patyczek_L2.gif"),
			PhotoImage(file="patyczek_L3.gif")
			]
			
		self.obrazki_prawa = [
			PhotoImage(file="patyczek_P1.gif"),
			PhotoImage(file="patyczek_P2.gif"),
			PhotoImage(file="patyczek_P3.gif")
			]	
		
		self.obrazki_stoi = [PhotoImage(file="patyczek_stoi.gif")]
		self.image = gra.plotno.create_image(200,400, image=self.obrazki_stoi[0], anchor="nw")
	...
	def animuj(self):
		...
		if self.x < 0:
			if self.y != 0:
				self.gra.plotno.itemconfig(self.image,	\
					image = self.obrazki_lewa[2])
			else:	
				self.gra.plotno.itemconfig(self.image,	\
					image = self.obrazki_lewa[self.biezacy_obrazek])
					
		elif self.x > 0:
			if self.y != 0:
				self.gra.plotno.itemconfig(self.image,	\
					image = self.obrazki_prawa[2])
			else:	
				self.gra.plotno.itemconfig(self.image,	\
					image = self.obrazki_prawa[self.biezacy_obrazek])
				
		elif self.x == 0:
			self.gra.plotno.itemconfig(self.image,	\
				image = self.obrazki_stoi[0])
	...
	def move(self):
		...
		''' Srawdzenie czy posać udeżyła w dolną lub górną krawędź płótna'''
		...
		''' Srawdzenie czy posać udeżyła w lewą lub prawą krawędź płótna'''
		...
		''' Srawdzenie czy posać udeżyła w jakiś obiekt'''
		for duszek in self.gra.duszki:
			...
			if dol and spadanie and self.y == 0	\
					and wsp.y2 < self.gra.wysokosc_plotna	\
					and kolizja_dol(1, wsp, duszek_wsp):
				spadanie = False
					
			if lewa and self.x < 0 and kolizja_lewa(wsp, duszek_wsp):
				self.x = 0
				lewa = False
				if duszek.koniec_gry:
					self.gra.bieganie = False
...
class DuszekDrzwi(Duszek):
	def __init__(self, gra, obrazek, x, y, szerokosc, wysokosc):
		...
		self.wspolrzedne = Coords(x, y, x + (szerokosc/2), y + szerokosc)
